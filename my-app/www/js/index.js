...
function getAllImageFromSD() {
	navigator.notification.alert('inside GetAllImageFromSD', // message
	function() {
	}, // callback
	'title', // title
	'ok' // buttonName
...
function onFileSystemSuccess(fileSystem) {
	navigator.notification.alert('inside onFileSystemSuccess', // message
	function() {
	}, // callback
	'title', // title
	'ok' // buttonName
	);
	fileSystem.root.getDirectory("/sdcard", {
		create : false,
		exclusive : false
	}, getDirSuccess, fail);
}
...
function getDirSuccess(dirEntry) {
	navigator.notification.alert('inside getDirSuccess', // message
	function() {
	}, // callback
	'title', // title
	'ok' // buttonName
...
function readerSuccess(entries) {
	...
	for (; i < len; i++) {
		if (entries[i].isFile) {
			if ((entries[i].name.indexOf(".jpeg") != -1)
					|| (entries[i].name.indexOf(".png") != -1)
	...
	readerTimeout = window
			.setTimeout(weAreDone, millisecondsBetweenReadSuccess);
}

function weAreDone() {

	navigator.notification.alert('inside weAreDone', // message
	function() {
	}, // callback
	'title', // title
	'ok' // buttonName
	...
	for (var j = 0; j < ImageFilePath.length; j++) {
		...
		var ChkId = "chk" + j;
		GalleryImageTag += "<img class='imggallery' id='" + j + "' src=' "
				+ ImageFilePath[j] + "'alt=''/>";
		GalleryImageTag += "<input id='" + ChkId + "' name='" + ChkId + "' type='checkbox' value='" + ImageFilePath[j]
				+ "' class='allignchk'>";
		GalleryImageTag += "</div>";
